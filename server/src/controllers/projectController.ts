...
export const getProjects = asyncHandler(async (req: Request, res: Response, next: NextFunction) => {
  // Manejar status que puede venir como array o string
  let statusFilter: ProjectStatus[] | undefined;
  if (req.query.status) {
    if (Array.isArray(req.query.status)) {
      statusFilter = req.query.status as ProjectStatus[];
    } else {
      statusFilter = (req.query.status as string).split(',') as ProjectStatus[];
    }
  }
